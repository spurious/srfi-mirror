...
(import
  (rnrs)
  (xitomatl srfi private make-alias-libraries-compat) 
  (xitomatl srfi private registry)
  (xitomatl srfi eager-comprehensions)
  (xitomatl IU-match)) ;; the I.U. match that came with Ikarus < 0.0.4
          ...
          (lambda (oa)
            ...
            (with-output-to-file fn
              (lambda ()
                ...
                (pretty-print 
                 `(library (xitomatl srfi ,oa)
                    (export ,@exports)
                    (import (xitomatl srfi ,primary-name)))))))
          output-aliases))]))
 ; construct: 
